...
function showStatus() {
  if (!todos.length) {
    statusText.textContent = "Bạn không có nhiệm vụ nào. 😊";
    return;
  }
  ...
  if (pending === 0) {
    ...
    statusText.textContent = "1 nhiệm vụ còn lại";
  } else {
    statusText.textContent = `${pending} / ${todos.length} nhiệm vụ còn lại`;
  }
}
...
function addTodo(e) {
  ...
  if (!newTodoText) {
    alert("Vui lòng nhập việc cần làm hợp lệ");
    return;
  }
