    ...
    async def dispatch(self, request: Request, call_next):
        ...
        if len(self.ip_records[client_ip]) >= RATE_LIMIT_REQUESTS:
            if path.startswith("/api/"):
                return JSONResponse(
                    status_code=429,
                    content={"status": "error", "error": {"title": "Hệ thống quá tải", "message": "Nghỉ tay 60 giây nhé!"}}
                )
            else:
                html_content = """
                <!DOCTYPE html>
                <html lang="vi"><head><meta charset="UTF-8"><title>429 - Quá tải</title></head>
                <body style="background: #0f2027; color: white; text-align: center; padding-top: 20%;">
                    <h1>⚠️ HỆ THỐNG QUÁ TẢI</h1><p>Sếp thao tác nhanh quá, hệ thống đang làm mát (60s)...</p>
                </body></html>
                """
